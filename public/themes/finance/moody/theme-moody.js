...
/* ============================================================
   ...
   - Hover SFX are discovered from /sounds/moody/ when available.
   ============================================================ */
...
const THEME_SOUND_ROOT = "/sounds/moody/";

function setIntroDance(isPlaying) {
    if (!root) return;
    root.classList.toggle("moody-intro-playing", Boolean(isPlaying));
}
...
async function discoverHoverSounds() {
    ...
    try {
        const response = await fetch("/api/theme-moody-sounds", { cache:"no-store" });
        if (response.ok) {
...
function playSynthHover() {
    ...
    osc.frequency.exponentialRampToValueAtTime(613.60, now + .18);
...
const ASSET_ROOT="/svg/theme-moody/";
const ASSETS=[
    "moody-01-sad-sun-face.svg",
    "moody-02-green-swirl-face.svg",
    "moody-03-mushroom-girl.svg",
    "moody-04-flower-girl.svg",
    "moody-05-blue-moon-face.svg",
    "moody-06-rose-girl.svg",
    "moody-07-twin-cherries-face.svg",
    "moody-08-speckled-moon-face.svg",
    "moody-09-sunflower-face.svg",
    "moody-10-strawberry-face.svg",
    "moody-11-rose-face.svg",
    "moody-12-small-flower-face.svg",
    "moody-13-rainbow-eyes.svg",
    "moody-14-pink-heart-face.svg",
    "moody-15-pink-mushroom-face.svg",
    "moody-16-large-flower-face.svg",
    "moody-17-cloud-face.svg",
    "moody-18-colorful-crescent-moon.svg"
];
...
function backdrop(){for(const cls of ["glow","pattern","accents","floor"]){const d=document.createElement("div");d.className=`theme-moody-${cls}`;root.appendChild(d)}const a=root.querySelector(".theme-moody-accents");for(let i=0;i<14;i++){const d=document.createElement("span");d.className="theme-moody-accent-dot";d.style.left=`${rand(2,98)}%`;d.style.top=`${rand(5,96)}%`;d.style.setProperty("--z",`${rand(8,28)}px`);d.style.setProperty("--dx",`${rand(-18,18)}px`);d.style.setProperty("--dy",`${rand(-20,20)}px`);d.style.setProperty("--dur",`${rand(4.5,8.5)}s`);d.style.setProperty("--delay",`${-rand(0,8)}s`);a.appendChild(d)}}
function burst(x,y){for(let i=0;i<7;i++){const s=document.createElement("span");s.className="theme-moody-burst";const ang=Math.PI*2*i/7+rand(-.22,.22),dist=rand(34,68);s.style.left=`${x}px`;s.style.top=`${y}px`;s.style.setProperty("--bx",`${Math.cos(ang)*dist}px`);s.style.setProperty("--by",`${Math.sin(ang)*dist}px`);s.style.setProperty("--bs",`${rand(5,11)}px`);root.appendChild(s);setTimeout(()=>s.remove(),720)}}
function makeItems(){const files=shuffle(ASSETS),ss=slots(files.length);files.forEach((f,i)=>{const d=document.createElement("div"),im=document.createElement("img");d.className="theme-moody-item";d.setAttribute("aria-hidden","true");im.src=ASSET_ROOT+encodeURIComponent(f);im.alt="";im.draggable=false;im.decoding="async";d.style.left=`${ss[i].x+rand(-4.2,4.2)}%`;d.style.top=`${ss[i].y+rand(-3,3)}%`;d.style.setProperty("--w",`${rand(155,240)}px`);d.style.setProperty("--s",rand(.92,1.14).toFixed(2));d.style.setProperty("--r",`${rand(-8,8)}deg`);d.style.setProperty("--dx",`${rand(-12,12)}px`);d.style.setProperty("--dy",`${rand(-14,14)}px`);d.style.setProperty("--dur",`${rand(6.5,10.5)}s`);d.style.setProperty("--delay",`${-rand(0,9)}s`);if(i%3===0||i%5===0){d.classList.add("theme-moody-intro-dancer");d.style.setProperty("--dance-delay",`${-rand(0,.8)}s`);d.style.setProperty("--dance-height",`${rand(12,20)}px`);d.style.setProperty("--dance-duration",`${rand(.62,.86)}s`)}d.appendChild(im);root.appendChild(d);items.push({el:d,last:0,hovered:false})})}
function interact(){mouse=e=>{const x=e.clientX,y=e.clientY;if(raf)cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{if(root){root.style.setProperty("--px",`${x}px`);root.style.setProperty("--py",`${y}px`);root.style.setProperty("--parx",`${((x/Math.max(innerWidth,1))-.5)*12}px`);root.style.setProperty("--pary",`${((y/Math.max(innerHeight,1))-.5)*9}px`)}raf=null});const now=performance.now();items.forEach(o=>{const r=o.el.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2;const inside=Math.hypot(cx-x,cy-y)<=92;if(inside&&!o.hovered){o.hovered=true;o.last=now;o.el.classList.remove("theme-moody-react");void o.el.offsetWidth;o.el.classList.add("theme-moody-react");burst(cx,cy);playHoverSound();setTimeout(()=>o.el?.classList.remove("theme-moody-react"),620)}else if(!inside&&o.hovered){o.hovered=false;fadeHover(240)}})};window.addEventListener("mousemove",mouse,{passive:true})}
export function mount(){if(root)return;root=document.createElement("div");root.id="moody-background";root.setAttribute("aria-hidden","true");document.body.appendChild(root);discoverHoverSounds();playIntro();backdrop();makeItems();interact()}
